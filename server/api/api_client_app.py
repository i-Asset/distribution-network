...
api_client_app = Blueprint("api_client_app", __name__)


@api_client_app.route(f"{prefix}/client_apps")
@api_client_app.route(f"{prefix}/client_apps/")
@api_client_app.route(f"{prefix}/client_apps/<string:any>")
@api_client_app.route(f"{prefix}/client_apps/<string:any>/")
def clients_no_sys(any=None):
    return jsonify({"value": "Please specify an user id and a system name.",
                    "url": f"{prefix}/client_apps/<string:user_id>/<string:system_name>", "status_code": 406}), 406
...
def client_per_system(user_id, system_url, client_name):
    ...
    # 1) extract the header content with the keys: Host, User-Agent, Accept, Authorization
    #    check if the user is allowed to get the systems (user_id < 0 -> Panta Rhei, user_id > 0 -> identity-service
    fct = f"{prefix}/client_apps/<string:user_id>/<string:system_url>/<string:client_name>"
    authorized, msg, status_code = authorize_request(fct=fct, user_id=user_id)
    if not authorized:
        return jsonify({"value": msg, "url": fct, "status_code": status_code}), status_code
...
@api_client_app.route(f"{prefix}/client_apps/<string:user_id>/<string:system_url>", methods=['POST', 'PUT'])
...
@api_client_app.route(f"{prefix}/delete_client_app/<string:user_id>/<string:system_url>/<string:client_name>",
                      methods=['DELETE'])
